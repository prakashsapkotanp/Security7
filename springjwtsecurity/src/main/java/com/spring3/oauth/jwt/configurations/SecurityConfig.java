...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf().disable()
                .authorizeRequests()
                .requestMatchers("/api/v1/user/login", "/api/v1/user/signup", "/api/v1/refreshToken","api/requests/getRequestByRequesterId/{requesterId}", "/api/v1/members/{id}","/api/v1/user/getId/{pnone_no}", "/api/v1/requesters/{user_id}","/api/requests/fullFillRequest/{requestId}/{userId}").permitAll()
